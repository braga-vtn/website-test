...
export const WorldMap = React.memo(function WorldMap({
  ...
  const paths = useMemo(() =>
    dots.map((dot, i) => {
      ...
      // Use gradient id based on theme
      const gradientId = `path-gradient-${theme}`;
    ...
    }), [dots, theme]);  // Ensure the paths update if theme changes
    ...
    <div className="w-full aspect-[2/1] bg-transparent rounded-lg relative font-sans">
      ...
      >
        ...
        {dots.map((dot, i) => {
          ...
          return (
            <PointGroup start={start} end={end} lineColor={lineColor} key={`points-group-${i}`} id={i} />
          );
        })}
      </svg>
    </div>
...
const PointGroup = React.memo(({ start, end, lineColor, id }: any) => (
  <g key={`points-group-${id}`}>
    <g key={`start-${id}`}>
